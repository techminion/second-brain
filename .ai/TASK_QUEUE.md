...
| CI-01 | CI: typecheck/lint/format/tests on PR | P0 | M | SETUP-04, SETUP-09 | backend | In Review | M0 | All four checks gate PRs. **Note:** CI must pin Node ≥ 22.12 (package `engines` floor set in DB-16 — supabase-js needs native WebSocket; Node 20 fails with a confusing error) |
