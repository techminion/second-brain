...
| DB-16 | Typed Supabase client factories | P1 | M | DB-01, SETUP-06 | database | In Review | M0 | Browser/session/service-role factories in `shared/lib`. **Known issue to resolve here:** `env.ts` reads `process.env[name]` dynamically — Next.js only inlines *static* `process.env.NEXT_PUBLIC_*` references into client bundles, so `getPublicEnvironment()` will throw in the browser. The browser factory must use static references (see 2026-07-16 review entry in [AI_HANDOFF](../docs/AI_HANDOFF.md)). **Also delivers:** the Cloud integration-test harness (isolated test users + cleanup, per [04_DATABASE §11](../docs/04_DATABASE.md#11-migration-strategy)) with the `profiles` cross-user denial test as its first test — the test DB-02 owes (GOV-6). Service-role usage in the harness and the `@supabase/supabase-js` + `@supabase/ssr` dependencies are approved by ADR-12 |
