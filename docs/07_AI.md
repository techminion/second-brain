...
This document specifies how Second Brain uses OpenAI's Responses API ([01_PRODUCT.md, MVP Tech Stack](../.claude/ARCHITECT_PROMPT.md)) for two things: turning note content into embeddings, and turning a user's question plus retrieved context into a cited, streamed answer. Everything here satisfies FR-SEM-1–3 and FR-AI-1–5 ([02_PRD.md §4.11–4.12](02_PRD.md#411-semantic-search)) within the latency budgets in [02_PRD.md §6](02_PRD.md#6-non-functional-requirements).
...
**Mid-stream failure:** if the upstream call fails after streaming has started, the server emits a terminal error event rather than silently closing the connection, and persists whatever partial `ChatMessage` content was streamed so far (marked incomplete) rather than discarding it — a user shouldn't lose a mostly-good answer because the last few tokens failed.
...
| Surface | Approach |
|---|---|
| Interactive chat (`AIService.streamChat`) | Per-user rate limit (token-bucket), enforced before the OpenAI call — protects against both cost blowout and a single user starving shared capacity. Exact thresholds are [09_SECURITY.md](DOCUMENT_INDEX.md#09_securitymd-planned)'s to set. |
...
- [09_SECURITY.md](DOCUMENT_INDEX.md#09_securitymd-planned) — concrete rate-limit thresholds referenced in §10.
